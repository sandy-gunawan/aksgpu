...
> **Naming note**: Resource names like `acrgpuweatherro6n`, `stgpuweatherro6n`, `kv-gpu-wea-ro6n` contain a 4-character random suffix generated by `setup-infrastructure.ps1` and saved to `.infra-config.json`. The suffix changes when you recreate the cluster from scratch. The current run uses `ro6n`. Public IPs and VNet/NSG identifiers are also assigned at cluster-create time; the values shown below reflect the current deploy.
...
graph TB
    subgraph Internet
        USER[/"👤 User Browser<br/>http://20.7.236.50"/]
    ...
    subgraph AZ["☁️ Azure Subscription: ME-MngEnvMCAP964184 (eastus2)"]
        ...
        subgraph RG["📦 Resource Group: rg-gpu-weather"]
            ACR["📦 ACR<br/>acrgpuweatherro6n.azurecr.io<br/>(Container Registry - Basic)<br/>Images: weather-api:v1, weather-ui:v1, crop-api:v1"]
            STORAGE["💾 Blob Storage<br/>stgpuweatherro6n<br/>(Standard_LRS, MI-only auth)"]
            KV["🔑 Key Vault<br/>kv-gpu-wea-ro6n"]
            ...
            subgraph AKS["⎈ AKS Cluster: aks-gpu-weather"]
                ...
                subgraph CPUPOOL["nodepool1 (System)<br/>Standard_DC4ds_v3 × 1<br/>4 vCPU, 32 GB RAM"]
                    INGRESS["🌐 Ingress Controller<br/>webapprouting (nginx)<br/>Public IP: 20.7.236.50"]
                ...
                subgraph GPUPOOL["gpupool1 (User)<br/>Standard_NC4as_T4_v3 × 0-1<br/>4 vCPU, 28 GB RAM, T4 16GB GPU<br/>Autoscale 0↔1<br/>Taint: sku=gpu:NoSchedule<br/>30 GB Ephemeral OS disk"]
        ...
        subgraph MCRG["📦 MC_rg-gpu-weather_aks-gpu-weather_eastus2<br/>(AKS-Managed Infrastructure)"]
            VNET["🔗 VNet<br/>aks-vnet-<id>"]
            NSG["🛡 NSG<br/>aks-agentpool-<id>-nsg"]
            LB["⚖️ Load Balancer<br/>kubernetes"]
            PUBIP["🌐 Public IP<br/>20.7.236.50"]
...
| `acrgpuweatherro6n` | Container Registry | Basic | Stores Docker images (weather-api:v1, weather-ui:v1) |
| `stgpuweatherro6n` | Storage Account | Standard_LRS | Blob storage for models, weather data cache, predictions |
| `kv-gpu-wea-ro6n` | Key Vault | Standard | Secrets management |
...
| `aks-vnet-<id>` | Virtual Network | Cluster networking (Azure CNI) |
...
| Public IP (20.7.236.50) | Public IP | Ingress entry point |
...
|----------|-------|------|---------|
| Kubelet MI (`4471-****-****-****-************`) | `stgpuweatherro6n` | Storage Blob Data Contributor | Pod access to Blob Storage without keys |
| CLI User | `stgpuweatherro6n` | Storage Blob Data Contributor | Script/manual blob access |
| Kubelet MI | `acrgpuweatherro6n` | AcrPull (via --attach-acr) | Pull container images from ACR |
...
Internet (User Browser)
    │
    ▼
Public IP: 20.7.236.50 ──► Azure Load Balancer
    │
    ▼
...
graph TB
    subgraph SUB["Azure Subscription<br/>ME-MngEnvMCAP964184-sandygunawan-1<br/>ID: 5a7c-****-****-****-************"]
        ...
        subgraph RG1["Resource Group: rg-gpu-weather<br/>📍 East US 2"]
            ...
            ACR["📦 Container Registry<br/><b>acrgpuweatherro6n.azurecr.io</b><br/>SKU: Basic │ Admin: disabled<br/>Images:<br/>  weather-api:v1<br/>  weather-ui:v1"]

            STORAGE["💾 Storage Account<br/><b>stgpuweatherro6n</b><br/>SKU: Standard_LRS │ Kind: StorageV2<br/>TLS: 1.2 │ HTTPS only<br/>Shared Key Access: ❌ disabled (policy)<br/>Auth: Managed Identity only<br/>Endpoint: stgpuweatherro6n.blob.core.windows.net"]

            KV["🔑 Key Vault<br/><b>kv-gpu-wea-ro6n</b><br/>RBAC Authorization: ✅ enabled<br/>Soft Delete: ✅ enabled<br/>Access: RBAC-based (no access policies)"]
        ...
        subgraph RG2["Resource Group: MC_rg-gpu-weather_aks-gpu-weather_eastus2<br/>📍 East US 2 │ AKS-Managed (do not modify)"]
            ...
            VNET["🔗 Virtual Network<br/><b>aks-vnet-<id></b><br/>Azure CNI networking"]

            NSG["🛡 Network Security Group<br/><b>aks-agentpool-<id>-nsg</b><br/>Inbound: HTTP(80), HTTPS(443)<br/>Outbound: Allow all"]

            LB["⚖ Load Balancer<br/><b>kubernetes</b><br/>SKU: Standard<br/>Frontend: Public IP"]

            PUBIP1["🌐 Public IP<br/><b>20.7.236.50</b><br/>Ingress entry point"]
            ...
            VMSS_GPU["🎮 VM Scale Set (GPU)<br/><b>aks-gpupool1-*-vmss</b><br/>SKU: Standard_NC4as_T4_v3<br/>Instances: 0-1 (autoscale)<br/>4 vCPU │ 28 GB RAM │ T4 16 GB GPU<br/>OS: Ubuntu 22.04 │ containerd<br/>30 GB Ephemeral OS disk<br/>Taint: sku=gpu:NoSchedule"]
        ...
        subgraph BLOB["Blob Storage Containers<br/>stgpuweatherro6n.blob.core.windows.net"]
        ...
        subgraph RBAC_SECTION["RBAC Role Assignments on Storage"]
            RBAC1["ServicePrincipal → <b>Storage Blob Data Contributor</b><br/>Kubelet MI (8b45-****) → stgpuweatherro6n<br/>Allows pods to read/write blobs without keys"]
            RBAC2["User → <b>Storage Blob Data Contributor</b><br/>CLI User (50d5-****) → stgpuweatherro6n<br/>Allows script/manual blob operations"]
        end
    end
...
|------|-----|----------|-------------|---------|
| Internet | Public IP (20.7.236.50) | HTTP :80 | None (public) | User access to web app |
...
│       ├── Object ID: 8b45-****-****-****-************                  │
│       ├── Client ID: 4471-****-****-****-************                  │
│       ├── ROLE: Storage Blob Data Contributor → stgpuweatherro6n       │
│       ├── ROLE: AcrPull → acrgpuweatherro6n (via --attach-acr)        │
...
│  of hourly data  │     │  Image: acrgpuweatherro6n.azurecr.io/       │
└──────────────────┘     │         weather-api:v1                      │
...
kubectl run train-tokyo -n gpu-weather --rm -it --restart=Never \
  --image=acrgpuweatherro6n.azurecr.io/weather-api:v1 \
